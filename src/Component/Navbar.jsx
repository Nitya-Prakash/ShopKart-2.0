...
const Navbar = () => {
    ...
    return (
        <nav className='w-[20%] h-full bg-zinc-100 flex flex-col items-center pt-5'>
            ...
            <div className='w-[85%] '>
                <Link to={"/"} className='mb-2 flex items-center gap-2 text-lg text-teal-700 font-semibold'>
                    <span style={{ backgroundColor: color() }} className='block w-[15px] h-[15px] rounded-full'></span>
                    Home
                </Link>
                {distinct_category.map((category, index) => (
                    <Link key={index} to={`/?category=${category}`} className='mb-2 flex items-center gap-2 text-lg'>
                        <span style={{ backgroundColor: color() }} className='block w-[15px] h-[15px] rounded-full'></span>
                        {category}
                    </Link>
                ))}
